...
void display()
{
  ...
  view = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -0.75f, -5.0f)); //-5.0f -> distance de la camera

  // Rotation du modèle pour le voir en 3D.
  model = glm::rotate(glm::mat4(1.0f), glm::radians( r ), glm::vec3(0.0f, 0.5f, 0.0f));
  ...
  // Dessin de tous les triangles du cube.
  glBindVertexArray(vaoids[0]);
  glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_SHORT, 0); // 36 indices pour 12 triangles (6 faces)

  // Dessin du second cube.

  // Le deuxième cube est décalé sur l'axe X, mais conserve la même rotation.
  glm::mat4 secondModel = glm::translate(model, glm::vec3(1.5f, 0.0f, 0.0f)); // Décalage sur l'axe X.

  // Recalcul de la matrice mvp pour le second cube.
  mvp = proj * view * secondModel;
  glUniformMatrix4fv(mvpid, 1, GL_FALSE, &mvp[0][0]);
  ...
  // Le 3eme cube est décalé sur l'axe X, mais conserve la même rotation.
  glm::mat4 troisiemeModel = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 1.5f, 0.0f)); // Décalage sur l'axe Y.

  // Recalcul de la matrice mvp pour le 3eme cube.
  mvp = proj * view * troisiemeModel;
  glUniformMatrix4fv(mvpid, 1, GL_FALSE, &mvp[0][0]);

  // Dessin des 2 cubes orbites.
  glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_SHORT, 0);

  // Le 3eme cube est décalé sur l'axe X, mais conserve la même rotation.
  glm::mat4 premierOrbite = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 1.5f, 0.0f)); // Décalage sur l'axe x.
  premierOrbite = glm::rotate(premierOrbite, glm::radians( r ), glm::vec3(0.5f, 0.0f, 0.0f));
  premierOrbite = glm::translate(premierOrbite, glm::vec3(0.0f, 0.75f, 0.0f)); // Décalage sur l'axe y.
  premierOrbite = glm::scale(premierOrbite, glm::vec3(0.5f, 0.5f, 0.5f));



  // Recalcul de la matrice mvp pour le 1er orbite.
  mvp = proj * view * premierOrbite;
  glUniformMatrix4fv(mvpid, 1, GL_FALSE, &mvp[0][0]);
  // Dessin du 1er orbite.
  glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_SHORT, 0);

  glm::mat4 deuxiemeOrbite = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 1.5f, 0.0f)); // Décalage sur l'axe Y.
  deuxiemeOrbite = glm::rotate(deuxiemeOrbite, glm::radians( r ), glm::vec3(0.0f, 0.5f, 0.0f));
  deuxiemeOrbite = glm::translate(deuxiemeOrbite, glm::vec3(0.75f, 0.0f, 0.0f)); // Décalage sur l'axe x.
  deuxiemeOrbite = glm::scale(deuxiemeOrbite, glm::vec3(0.5f, 0.5f, 0.5f)); // Décalage sur l'axe x.


  // Recalcul de la matrice mvp pour le 1er orbite.
  mvp = proj * view * deuxiemeOrbite;
  glUniformMatrix4fv(mvpid, 1, GL_FALSE, &mvp[0][0]);
  // Dessin du 1er orbite.
  glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_SHORT, 0);
...
std::vector<float> vertices = {
  ...
  std::vector<unsigned short> indices = {
      ...
      0, 1, 5, 5, 4, 0, // Face dessous
      2, 3, 7, 7, 6, 2, // Face dessus
