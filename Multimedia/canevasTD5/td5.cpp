...
std::array< float, 3 > eye = { 0.0f, 0.0f, 5.0f };
    ...
    {
    shaderProg shader;
    unsigned int vaoids; // VaoID qui contient les VBO oÃ¹ ce trouve ce maillage (et ses normales etc..) dans la VRAM
    unsigned int nbtriangles;
    float angle = 0.0f;
    float scale = 0.0f; // stocke l'opÃ©ration de normalisation de la taille du maillage (rÃ©alisÃ© lors de la lecture du maillage)
    float inc = 0.1f;
    float x, y, z; // stocke la position du centre du  maillage (rÃ©alisÃ© lors de la lecture du maillage) il faut utiliser cette info lors du dessin afin de toujours veiller Ã  recentrer le maillage en (0,0,0)
